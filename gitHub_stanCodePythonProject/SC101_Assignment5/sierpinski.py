...
def main():
	"""
	Use sierpinski_triangle to do recursion and draw the triangle by order set.
	"""
...
def sierpinski_triangle(order, length, upper_left_x, upper_left_y):
	"""
	:param order: the Sierpinski Triangle order left
	:param length: the length of the triangle
	:param upper_left_x: the x position
	:param upper_left_y: the y position
	:return: no return
	"""
	if order != 0:
		order -= 1
		...
		sierpinski_triangle(order, length / 2, upper_left_x+(length/4), upper_left_y+(length*0.433))
